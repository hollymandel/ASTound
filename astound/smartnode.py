...
jedi.settings.fast_parser = False # otherwise jedi will only maintain one script object! 
...
class Node:
    ...
    def __init__(self, ast_node = None):
        if any(isinstance(ast_node, forbidden_type) for forbidden_type in FORBIDDEN_TYPES):
    ...
    def __repr__(self):
        ...
        if isinstance(self._ast_node, ast.Assign):
            return (
                "Assignment: (" + 
                ", ".join([Node(target).__repr__() for target in self._ast_node.targets]) +
                ") = (" +
...
class SmartNode(Node):
    ...
    def infer_inheritance(self):
        ...
        if len(self._ast_node.bases) > 1:
            logging.warning("ASTound cannot resolve multiple inheritance. Defaulting to first parent class.")
    ...
    def attach_child(self, line, col):
        ...
        self._children[(line, col)] = SmartNode(self.get_child_by_loc(line, col), source = self._source, parent = self)
    ...
    def summarize(self, client):
        ...
        individual_prompt = (
            INDIVIDUAL_SUMMARY_HEADER + 
            "".join(self.get_text())
        )

        individual_summary = client.messages.create(
            **CLAUDE_KWARGS, 
            messages=[
        ...
        if len(self._children) == 0:
            ...
            joint_prompt = (
                ...
                individual_summary + 
                "\n".join([f"{child_header(x)}{x.summarize(client)}" for x in self._children.values()])
            )
    
            joint_summary = client.messages.create(
                **CLAUDE_KWARGS, 
                messages=[
