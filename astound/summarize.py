...
def summarize(node: SmartNode, client, prompts: Optional[Union[MappingProxyType, Dict]] = None):
    ...
    if len(node._children) == 0:
        ...
        joint_prompt = (
            ...
            individual_summary + 
            "\n".join([f"{child_header(x)}{x.summarize(client)}" for x in node._children.values()])
        )
